...
public:
template<class T1> // because we are using template feature so according to mentor and docs it will not become part of library because we know complier will write those appropritate function for us,acc to user called
void set(string keyName,T1 something,bool *success,__container_operation_failure_reason__ *reason)
{
...
template<class T2>
void get(string keyName,T2 anything,bool *success,__container_operation_failure_reason__ *reason)
{
auto iterator=dataSet.find(keyName);
if(iterator==dataSet.end())
{
...
template<class T3>
void remove(string keyName,T3 anything,bool *success,__container_operation_failure_reason__ *reason)
{
auto iterator=dataSet.find(keyName);
if(iterator==dataSet.end())
{
...
string name;
string value;
string expiresOn; // when web-developer specified both then
long maxAge; // I will be give priority to maxAge Factor
string domain;
string path;
bool isSecure;
bool isHttpOnly;
...
HeaderUtility(){}
public:
static void parseHeader(const char *header,map<string,string> &headerFieldMap)
{
...
Request(char *method,char *requestURI,char *httpVersion,char *dataInRequest,map<string,string> &headerFieldsMap):headerFieldsMap(headerFieldsMap)
{
this->method=method;
...
if(fields==headerFieldsMap.end()) return string("");
